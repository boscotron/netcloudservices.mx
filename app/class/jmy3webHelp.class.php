...
class JMY3WEB extends JMY3MySQL{
  ...
  public function __construct(){
  	...
  	$tabla = "vistaweb";
	  global $modoEdicion;
	  $per = $this->modulos(['modulos_permisos'=>1]);
  ...
  //	parent::db([$tabla]); // Verificamos que exista la tabla, de nos er así el sistema la crea
	...
	public function url_inicio($d=[]){ if($d['return']){ return RUTA_ACTUAL; }else{echo RUTA_ACTUAL;}  }
  	public function cdn(){ return "https://cdn-concomsis.firebaseapp.com/"; }
	...
	public function modulos($d=[]){	
		...
		$p=$this->ver([
			"TABLA"=>TABLA_USUARIOS.'__'.$o['body']['api_web']['ID_F'], 
			"ID"=>$i,
			"COL"=>['modulos'], 
		]);
	...
	public function guardar_session($d=null){
		...
		if(is_array($s)){
			...
			$r=parent::guardar([
				"TABLA"=>TABLA_USUARIOS.'__'.$s['body']['api_web']['ID_F'], 
				"ID_F"=>$s['user']['user_id'],
				"A_D"=>TRUE, 
				"GUARDAR"=>[
					"perfil"=>$s['user'],
