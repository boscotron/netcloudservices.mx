...
function sanear_string($string)
{
  ...
  function smart_resize_image($file,
   ...
   $op=["height"=>($op2['height']!='' && $op2['height']!='undefined')?$op2['height']:IMG_RESIZING_HEIGHT,
    ...
    $op['porcional'] =($op['height']==0)?true:false;
    $targetPath = BASE_ARCHIVO.$op['width'].'x'.$op['height'].'_'.(( $op['porcional'] )?'por':'nopor').'_'.uniqid().'_'.sanear_string($_FILES['userImage']['name']);
  ...
  echo json_encode(["error"=>"Faltan archivos"]);
}
}
?>