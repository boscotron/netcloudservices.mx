...
function guardarSlider(d=[]){
	console.log('jmy_slider_guardar');
	jQuery(function ($) { 
	let t = [];
	  ...
	  for(let i=0;i<d.var.length;i++){
		  ...
		  switch (t.type) {
			case 'imagen':
				...
				h=h+'<div class="drop-area" data-id_target="'+t.id+'" data-class="'+t.class+'" data-page="'+d.page+'" data-idadd="'+t.idadd+'" data-tabla="'+d.tabla+'" data-width="'+t.width+'" data-background="'+t.background+'" data-height="'+t.height+'" style="'+style.wS_ima+'" ><h3 class="drop-text" style="'+style.wS_ima_h3+'">'+leng.drop_image+' '+t2+'</h3></div>';
				dA++;
			break;
			case 'text':
...
function uForm(formData,d=[]){
	...
	$(".jmy_slider_guardar").css("background-color","rgba(30,30,30,0.8)");
	...
	$.ajax({
		...
		success: function(r){
			...
			$(".jmy_slider_guardar").css("background-color","rgba(30,140,30,0.8)");
			...
			if(d.background!=undefined)
				$("."+d.background).css("background-image",'url("'+r.url+'") !important ');
